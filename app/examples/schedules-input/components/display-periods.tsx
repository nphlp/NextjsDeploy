...
export default function DisplayPeriods() {
    ...
    const overlappingSchedules: boolean = user.Schedules.some((schedule1, i) => {
        // Comparer avec les schedules suivants uniquement (évite les doublons et la comparaison avec soi-même)
        return user.Schedules.slice(i + 1).some((schedule2) => {
    ...
    return (
        <Card className="space-y-4">
            ...
            {user?.Schedules.map((schedule, index) => (
                <Schedule key={index} index={index} schedule={schedule} />
            ))}
