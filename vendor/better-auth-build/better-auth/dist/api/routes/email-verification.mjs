...
async function sendVerificationEmailFn(ctx, user) {
	...
	if (ctx.context.options.emailVerification?.onEmailVerificationRequested) await ctx.context.runInBackgroundOrAwait(ctx.context.options.emailVerification.onEmailVerificationRequested({ user }, ctx.request));
}
