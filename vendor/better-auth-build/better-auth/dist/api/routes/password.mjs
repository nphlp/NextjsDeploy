...
const requestPasswordReset = createAuthEndpoint("/request-password-reset", {
	...
	if (!user) {
		...
		*/
		...
		if (ctx.context.options.emailAndPassword?.sendResetPasswordNoAccount) await ctx.context.runInBackgroundOrAwait(ctx.context.options.emailAndPassword.sendResetPasswordNoAccount({ email }, ctx.request));
		return ctx.json({
			status: true,
	...
	if (ctx.context.options.emailAndPassword?.onResetPasswordRequested) await ctx.context.runInBackgroundOrAwait(ctx.context.options.emailAndPassword.onResetPasswordRequested({ user: user.user }, ctx.request));
	return ctx.json({
		status: true,
		message: "If this email exists in our system, check your email for the reset link"
	});
