...
const changePassword = createAuthEndpoint("/change-password", {
	...
	if (ctx.context.options.emailAndPassword?.onPasswordChanged) await ctx.context.runInBackgroundOrAwait(ctx.context.options.emailAndPassword.onPasswordChanged({ user: session.user }, ctx.request));
	return ctx.json({
		token,
...
const changeEmail = createAuthEndpoint("/change-email", {
	...
	*/
	const sendChangeEmailVerification = ctx.context.options.user?.changeEmail?.sendVerificationEmail;
	if (!sendChangeEmailVerification) {
		ctx.context.logger.error("Change email verification isn't configured.");
		throw APIError.fromStatus("BAD_REQUEST", { message: "Change email verification isn't configured. Please set user.changeEmail.sendVerificationEmail." });
	}
	...
	const expiresAt = new Date(Date.now() + (ctx.context.options.emailVerification?.expiresIn || 3600) * 1e3);
	...
	await ctx.context.internalAdapter.updateUser(ctx.context.session.user.id, { pendingEmail: newEmail });
	if (ctx.context.options.user?.changeEmail?.onChangeEmailRequested) await ctx.context.runInBackgroundOrAwait(ctx.context.options.user.changeEmail.onChangeEmailRequested({
	...
	if (sendChangeEmailVerification) try {
		...
		await ctx.context.internalAdapter.updateUser(ctx.context.session.user.id, { pendingEmail: null });
		await ctx.context.internalAdapter.deleteVerificationByIdentifier(identifier);
		throw e;
	}
	return ctx.json({ status: true });
});
const cancelEmailChange = createAuthEndpoint("/cancel-email-change", {
	method: "POST",
	...
	metadata: { openapi: {
		...
		description: "Cancel a pending email change and clear the pendingEmail field.",
		responses: { 200: {
	...
	if (!ctx.context.options.user?.changeEmail?.enabled) throw APIError.fromStatus("BAD_REQUEST", { message: "Change email is disabled" });
	await ctx.context.internalAdapter.deleteVerificationByIdentifier(`change-email:${ctx.context.session.user.id}`);
	await ctx.context.internalAdapter.updateUser(ctx.context.session.user.id, { pendingEmail: null });
	if (ctx.context.options.user?.changeEmail?.onChangeEmailCancelled) await ctx.context.runInBackgroundOrAwait(ctx.context.options.user.changeEmail.onChangeEmailCancelled({ user: ctx.context.session.user }, ctx.request));
	return ctx.json({ status: true });
});
const verifyEmailChange = createAuthEndpoint("/verify-email-change/:userId/:token", {
	method: "GET",
	query: z.object({ callbackURL: z.string().optional() }),
	use: [originCheck((ctx) => ctx.query?.callbackURL)],
	metadata: { openapi: {
		...
		responses: { 302: { description: "Email change applied successfully, redirects to callbackURL" } }
	} }
}, async (ctx) => {
	const { userId, token } = ctx.params;
	const callbackURL = ctx.query?.callbackURL || "/";
	const errorUrl = `${callbackURL}${callbackURL.includes("?") ? "&" : "?"}error=INVALID_TOKEN`;
	if (!userId || !token) throw ctx.redirect(errorUrl);
	const identifier = `change-email:${userId}`;
	const verification = await ctx.context.internalAdapter.findVerificationValue(identifier);
	if (!verification || verification.expiresAt < /* @__PURE__ */ new Date()) throw ctx.redirect(errorUrl);
	let data;
	try {
	...
	if (data.token !== token) throw ctx.redirect(errorUrl);
	...
	await ctx.context.internalAdapter.deleteVerificationByIdentifier(identifier);
	if (ctx.context.options.user?.changeEmail?.onChangeEmailCompleted) await ctx.context.runInBackgroundOrAwait(ctx.context.options.user.changeEmail.onChangeEmailCompleted({
		user: updatedUser,
		oldEmail: data.oldEmail,
		newEmail: data.newEmail
	}, ctx.request));
	if (ctx.context.options.user?.changeEmail?.revokeOtherSessions) await ctx.context.internalAdapter.deleteSessions(updatedUser.id);
	const session = await ctx.context.internalAdapter.createSession(updatedUser.id);
	if (session) await setSessionCookie(ctx, {
		session,
		user: updatedUser
	});
...
//#endregion
export { cancelEmailChange, changeEmail, changePassword, deleteUser, deleteUserCallback, setPassword, updateUser, verifyEmailChange };
